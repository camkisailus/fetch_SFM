...
if __name__ == '__main__':
    ...
    # fetch = RobotCommander()
    # arm = MoveGroupCommander('arm')
    # find_client = actionlib.SimpleActionClient('basic_grasping_perception/find_objects', FindGraspableObjectsAction)
    # find_client.wait_for_server()
    # rospy.sleep(1)
    # goal = FindGraspableObjectsGoal()
    # goal.plan_grasps = False
    # find_client.send_goal(goal)
    # find_client.wait_for_result()
    ...
    #     obj_pose.pose = obj.object.primitive_poses[0]
    #     obj_pose.header.frame_id = 'base_link'
    #     graspable_objs.append(GraspableObject(obj.object.name, obj_pose))
    #     rospy.loginfo("Adding {} to planning scene.".format(obj.object.name))
    #     scene.add_box(obj.object.name, obj_pose, (obj.object.primitives[0].dimensions[0],obj.object.primitives[0].dimensions[1], obj.object.primitives[0].dimensions[2]))
    # for i, obj in enumerate(graspable_objs):
    #     print("{})\n\t{}\n\t{}".format(i, obj.name, obj.pose))
    # choice = int(input("What object do you want to grasp?"))
    # obj_to_grasp = graspable_objs[choice]
    # grasp_pose = obj_to_grasp.pose
    # grasp_pose.pose.position.x -= 0.17
    # grasp_pose.pose.orientation.x = 0
    # grasp_pose.pose.orientation.y = 0
    # grasp_pose.pose.orientation.z = 0
    # grasp_pose.pose.orientation.w= 1
    # # print("Planning grasp to {} at {}".format(obj_to_grasp.name, grasp_pose.pose))
    # grasp = Grasp()
    # grasp.id = 'test'
    ...
    # grasp.allowed_touch_objects = [obj_to_grasp.name]
    # arm.pick(obj_to_grasp.name, grasp)
